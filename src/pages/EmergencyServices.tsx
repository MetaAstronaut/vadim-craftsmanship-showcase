        ...
        <div className="container-custom relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {emergencyServices.map((service, index) => (
              <Card key={index} className="group relative border-2 border-card-border hover:border-destructive/40 bg-card hover:shadow-premium-lg transition-all duration-500 hover:-translate-y-2">
                <CardContent className="p-8 text-center relative">
                  ...
                  <h3 className="text-2xl font-semibold mb-3 text-foreground relative z-10">{service.title}</h3>
      ...
      <section className="section-padding bg-gradient-to-b from-muted/30 via-destructive/5 to-muted/30 border-y border-destructive/20 relative overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="flex items-start gap-6 max-w-4xl mx-auto p-8 bg-card/50 backdrop-blur-sm rounded-2xl border-2 border-destructive/20">
            ...
            <div>
              <h3 className="font-serif text-3xl font-bold mb-3 text-foreground">Important Note About Emergencies</h3>
              <p className="text-lg text-muted-foreground leading-relaxed">
                If you're experiencing a life-threatening emergency (gas leak, electrical fire, severe flooding), 
                please contact 911 immediately. Once the immediate danger is resolved, we're here to help with repairs and restoration.
              </p>
            </div>
          </div>
        </div>
                ...
                <form className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input id="name" placeholder="John Smith" required className="h-12 border-2 border-input focus:border-secondary transition-colors" />
                    </div>
                    <div className="space-y-2">
                      <label htmlFor="email" className="text-sm font-medium text-foreground">
                        Email Address *
                      </label>
                      <Input id="email" type="email" placeholder="john@example.com" required className="h-12 border-2 border-input focus:border-secondary transition-colors" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="address" placeholder="123 Main St, Orlando, FL 32801" required className="h-12 border-2 border-input focus:border-secondary transition-colors" />
                  </div>

                  <div className="space-y-2">
                    <label htmlFor="emergency-type" className="text-sm font-medium text-foreground">
                      Type of Emergency *
                    </label>
                    <Input id="emergency-type" placeholder="e.g., Burst pipe, Electrical issue" required className="h-12 border-2 border-input focus:border-secondary transition-colors" />
                  </div>
